  ...
  return (
    <div className='homemain'>
      ...
      return(
       ...
       { item.ud==JSON.parse(localStorage.getItem("userd"))._id &&
      <div onClick={()=>{
       ...
       }} style={{position:"absolute", display:"inline",fontSize:"1.3em"}}><MdDelete /></div>}
        <p  style={{fontSize:"1.3em",display:"inline",marginLeft:"40px"}} key={index}><b>{item.name} </b>{item.comment}</p>
       </div>
      )
    }
      )}
